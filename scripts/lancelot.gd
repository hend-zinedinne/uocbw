...
@onready var mood_bubble: AnimatedSprite2D = $mood_module/mood_bubble
...
@onready var general_anim: AnimationPlayer = $general_anim
@onready var spell_barrier_anim: AnimationPlayer = $spell_barrier/spell_barrier_anim
@onready var mood_trigger_anim: AnimationPlayer = $mood_module/mood_trigger_anim
...
func _ready():
	...
	mood_bubble.modulate.a = 0

func _physics_process(delta):
	mood_bubble.play(str(mood))
	health = clamp(health,0, 100)
	speed = lerp(speed, target_speed, 5 * delta)
...
func launch_away():
	linear_velocity = (enemy_direction * -1)
...
func _on_lute_body_entered(body: Node2D) -> void:
	if body.has_method("take_damage"):
		...
		body.launch_away(weapon_damage)
...
func cast_spell(spell):
	...
	if spell == "barrier":
		...
		if !barrier_activated:
			...
			barrier_health = 5.0
			spell_barrier_anim.play("barrier")
			spell_barrier.get_child(0).text = str(int(barrier_health))
			spell_barrier.get_child(0).visible = true
		else:
			barrier_health += 5.0

func _on_weapon_hit() -> void:
	general_anim.play("flip_weapon")
...
func _on_spell_barrier_body_entered(body: Node2D) -> void:
	if body.is_in_group("ball") and body != self:
		...
		spell_barrier.get_child(0).text = str(int(barrier_health))
		if barrier_health <= 0:
			spell_barrier.get_child(0).visible = false
			spell_barrier_anim.play_backwards("barrier")
